...
export function CreateBOMDialog({ open, onOpenChange, onCreated }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = await response.json();
      if (!payload.success) {
        toast.error(payload.error || "Failed to create BOM");
        return;
      }
      ...
      console.error(error);
      toast.error("Network error while creating BOM");
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl p-0 border-2 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] overflow-hidden">
        <DialogHeader className="px-6 py-5 border-b bg-white">
          <DialogTitle className="text-3xl font-black uppercase tracking-tight">Create Bill Of Materials</DialogTitle>
