...
export function ClosingJournalDialog({ open, onOpenChange }: ClosingJournalDialogProps) {
    ...
    return (
        <NBDialog open={open} onOpenChange={handleOpenChange} size="wide">
            <NBDialogHeader icon={Lock} title="Jurnal Penutup" subtitle="Tutup akun pendapatan & beban, transfer laba bersih ke Laba Ditahan" />

            <NBDialogBody>
                {/* Year Selection */}
                <NBSection icon={Lock} title="Pilih Tahun Fiskal">
                    <div className="flex items-end gap-3">
                        ...
                        <Button
                            ...
                            disabled={loading}
                            className="bg-black text-white border border-black hover:bg-zinc-800 font-black uppercase text-[10px] tracking-wider px-5 h-8 rounded-none gap-1.5"
                        >
                            {loading ? (
                ...
                {preview && !preview.alreadyClosed && preview.lines.length > 0 && (
                    <>
                        ...
                        {/* Detail Lines */}
                        <NBSection icon={Lock} title="Preview Jurnal Penutup">
            ...
            {/* Footer — only show when preview has postable lines */}
            {preview && !preview.alreadyClosed && preview.lines.length > 0 && (
                <NBDialogFooter
                    onCancel={() => handleOpenChange(false)}
                    onSubmit={handlePost}
                    submitting={posting}
                    submitLabel="Posting Jurnal Penutup"
                    disabled={!preview.retainedEarningsAccount}
                />
            )}
        </NBDialog>
    )
