...
import {
    bulkImportMovements,
    type BulkImportMovementRow,
} from "@/app/actions/inventory"

// ─── Column mapping ─────────────────────────────────────────────────────────
const COLUMN_ALIASES: Record<string, keyof BulkImportMovementRow> = {
    // Kode Produk
    ...
    "jenis": "type",
    "movement type": "type",
    "tipe pergerakan": "type",
    // Jumlah
    "jumlah": "quantity",
    "quantity": "quantity",
    "qty": "quantity",
    // Gudang Tujuan
    "gudang tujuan": "targetWarehouseName",
    "target warehouse": "targetWarehouseName",
    "tujuan": "targetWarehouseName",
    "to warehouse": "targetWarehouseName",
    // Catatan
    "catatan": "notes",
    "notes": "notes",
    "keterangan": "notes",
...
// ─── Types ───────────────────────────────────────────────────────────────────
interface ParsedRow extends BulkImportMovementRow {
    _rowIndex: number
    _hasError: boolean
    _errorMsg?: string
}
    ...
    return new Promise((resolve) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                const headerMap: Record<number, keyof BulkImportMovementRow> = {}
                ...
                for (let i = 1; i < rawData.length; i++) {
                    ...
                    } else if (!mapped.warehouseName) {
                        mapped._hasError = true
                        mapped._errorMsg = "Gudang wajib diisi"
                    } else if (!mapped.type || !VALID_TYPES.has(String(mapped.type).toUpperCase())) {
                        mapped._hasError = true
                        mapped._errorMsg = `Tipe "${mapped.type || ''}" tidak valid`
                    } else if (!mapped.quantity || mapped.quantity <= 0) {
                        mapped._hasError = true
                        mapped._errorMsg = "Jumlah harus lebih dari 0"
                    } else if (String(mapped.type).toUpperCase() === "TRANSFER" && !mapped.targetWarehouseName) {
                        mapped._hasError = true
                        mapped._errorMsg = "Transfer memerlukan gudang tujuan"
                    }
...
function typeColor(t: string) {
    const map: Record<string, string> = {
        ADJUSTMENT_IN: "text-emerald-700",
        ADJUSTMENT_OUT: "text-red-700",
        TRANSFER: "text-violet-700",
        SCRAP: "text-amber-700",
    }
    return map[t?.toUpperCase()] || "text-zinc-700"
}
...
export function ImportMovementsDialog() {
    ...
    const handleImport = useCallback(async () => {
        ...
        const payload: BulkImportMovementRow[] = validRows.map(({ _rowIndex, _hasError, _errorMsg, ...rest }) => rest)

        setProgress(40)

        try {
            const result = await bulkImportMovements(payload)
            setProgress(100)
            ...
            if (result.imported > 0) {
                toast.success(`${result.imported} pergerakan stok berhasil diimport`, {
                    description: result.errors.length > 0
                        ? `${result.errors.length} baris gagal — lihat detail di dialog.`
                        : "Semua pergerakan berhasil dicatat.",
                })
            } else {
                toast.error("Import gagal — tidak ada pergerakan yang berhasil.", {
    ...
    const handleDownloadTemplate = useCallback(() => {
        const ws = XLSX.utils.aoa_to_sheet([
            ...
            ["CLN-002", "Gudang Utama", "ADJUSTMENT_OUT", 5, "", "Barang rusak"],
            ["KAO-001", "Gudang Utama", "TRANSFER", 10, "Gudang Cabang", "Pemindahan ke cabang"],
            ["CLN-002", "Gudang Utama", "SCRAP", 3, "", "Barang kadaluarsa"],
        ])
        const wb = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(wb, ws, "Template Import Pergerakan")
        XLSX.writeFile(wb, "template-import-pergerakan-stok.xlsx")
    }, [])
