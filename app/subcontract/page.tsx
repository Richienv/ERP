        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4">
                    ...
                    <div className={`text-2xl font-black ${data.overdueCount > 0 ? "text-red-600" : ""}`}>
                        {data.overdueCount}
                    </div>
                </div>
                ...
                <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4">
                    ...
                    <div className={`text-2xl font-black ${data.yieldRate >= 95 ? "text-emerald-600" : data.yieldRate >= 85 ? "text-amber-600" : "text-red-600"}`}>
                        {data.yieldRate}%
                    </div>
                ...
                <div className="bg-white border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] p-4">
                    ...
                    <div className={`text-2xl font-black ${data.onTimeDeliveryPercent >= 80 ? "text-emerald-600" : data.onTimeDeliveryPercent >= 50 ? "text-amber-600" : "text-red-600"}`}>
            ...
            {/* Status Distribution + Material at Vendor */}
                ...
                <div className="bg-red-50 border-2 border-red-400 shadow-[4px_4px_0px_0px_rgba(239,68,68,0.3)]">
                    ...
                    <div className="divide-y divide-red-200">
                        {data.overdueOrders.map((order) => (
                            <Link
                                key={order.id}
                                href={`/subcontract/orders/${order.id}`}
                                className="px-4 py-3 flex items-center gap-4 hover:bg-red-100 block"
                            >
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-black text-red-800">{order.number}</span>
                                        <span className={`text-[8px] font-black px-1.5 py-0.5 border ${subcontractStatusColors[order.status]}`}>
                                            {subcontractStatusLabels[order.status]}
                                        </span>
                                    </div>
                                    <div className="text-[10px] text-red-600 font-bold">
                                        {order.subcontractorName} — Target: {order.expectedReturnDate
                                ...
                                <div className="text-right shrink-0">
                                    <div className="text-xs font-mono font-bold text-red-800">
                                        {order.totalReturnedQty}/{order.totalIssuedQty}
                                    </div>
                            ...
                            >
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span
                                            className={`text-[8px] font-black px-1.5 py-0.5 border ${subcontractStatusColors[order.status]}`}
                                        >
                                            {subcontractStatusLabels[order.status]}
                                        </span>
                                    </div>
                                    <div className="text-[10px] text-zinc-500 font-bold">
                                        {order.subcontractorName} — {order.operation}
                                    </div>
                                </div>
                                <div className="text-right shrink-0">
