...
export default function APCheckbookPage() {
    ...
    async function loadData() {
        setLoading(true)
        const [payData, venData] = await Promise.all([
            getVendorPayments(),
            getVendors()
        ])
        setPayments(payData)
    ...
    const handleSign = () => {
        if (selectedVendorId && amount) {
            setIsSigned(true)
            toast.info("Authorization applied to check")
        } else {
            toast.error("Please select a vendor and amount first")
        }
    }

    const handleSubmit = async () => {
        if (!selectedVendorId || !amount || !isSigned) return
            ...
            <div className="flex-1 grid grid-cols-1 lg:grid-cols-12 gap-8 min-h-0 items-start">
                ...
                <div className="lg:col-span-7 space-y-6">
                    ...
                    <Card className="border-4 border-black bg-[#fdfdfd] shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] relative overflow-hidden">
                        ...
                        <CardContent className="p-8 md:pl-24 relative z-10 space-y-8">
                            ...
                            <div className="flex justify-between items-start">
                                ...
                                <div className="font-mono text-xl font-black tracking-widest text-black/20">NO. AUTH-PR-2024</div>
                            </div>
                    ...
                    <Button
                        ...
                        disabled={!isSigned || submitting || !amount || !selectedVendorId}
                    ...
                    >
                        {submitting ? (
                            ...
                            <span className="flex items-center gap-2">Execute Transfer <ChevronRight className="h-6 w-6" /></span>
                        ) : (
                            'Sign Check to Continue'
                        )}
                    </Button>
                </div>
