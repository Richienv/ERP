                ...
                <div className="flex items-center justify-between border-b pb-6 border-black">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-4xl font-black tracking-tight text-emerald-600">
                            {loading ? "..." : reportType === "pnl" && formatCurrency(pnlData?.netIncome || 0)}
                            {reportType === "bs" && formatCurrency(balanceSheetData?.assets?.totalAssets || 0)}
                            {reportType === "cf" && formatCurrency(cashFlowData?.netIncreaseInCash || 0)}
                        </p>
                        {!loading && (
                                        ...
                                        <Table>
                                            <TableBody>
                                                <TableRow className="font-black bg-zinc-50">
                                                    ...
                                                    <TableCell className="text-right font-mono">{formatCurrency(pnlData.revenue)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell className="pl-8">Cost of Goods Sold</TableCell>
                                                    <TableCell className="text-right font-mono text-red-600">({formatCurrency(pnlData.costOfGoodsSold)})</TableCell>
                                                </TableRow>
                                                <TableRow className="font-bold bg-zinc-100">
                                                    <TableCell>Gross Profit</TableCell>
                                                    <TableCell className="text-right font-mono text-blue-600">{formatCurrency(pnlData.grossProfit)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell className="pl-8">Operating Expenses</TableCell>
                                                    <TableCell className="text-right font-mono text-red-600">({formatCurrency(pnlData.totalOperatingExpenses)})</TableCell>
                                                </TableRow>
                                                {pnlData.operatingExpenses?.map((exp: any, idx: number) => (
                                                    <TableRow key={idx}>
                                                        <TableCell className="pl-12 text-sm text-zinc-500">{exp.category}</TableCell>
                                                        <TableCell className="text-right font-mono text-sm text-red-600">({formatCurrency(exp.amount)})</TableCell>
                                                    </TableRow>
                                                ))}
                                                <TableRow className="font-bold bg-zinc-100">
                                                    <TableCell>Operating Income</TableCell>
                                                    <TableCell className="text-right font-mono text-indigo-600">{formatCurrency(pnlData.operatingIncome)}</TableCell>
                                                </TableRow>
                                                <TableRow>
                                                    <TableCell className="pl-8">Tax Expense</TableCell>
                                                    <TableCell className="text-right font-mono text-red-600">({formatCurrency(pnlData.taxExpense)})</TableCell>
                                                </TableRow>
                                        ...
                                        <Table>
                                            <TableBody>
                                                <TableRow className="bg-emerald-50/50 font-bold">
                                                    ...
                                                    <TableCell className="text-right font-mono">{formatCurrency(balanceSheetData.assets?.totalCurrentAssets)}</TableCell>
                                                </TableRow>
                                                {balanceSheetData.assets?.currentAssets?.slice(0, 5).map((asset: any, idx: number) => (
                                                    <TableRow key={idx}>
                                                        ...
                                                        <TableCell className="text-right font-mono text-sm">{formatCurrency(asset.amount)}</TableCell>
                                                    </TableRow>
                                                ))}
                                                <TableRow className="bg-zinc-50 font-bold">
                                                    <TableCell>Fixed Assets</TableCell>
                                                    <TableCell className="text-right font-mono">{formatCurrency(balanceSheetData.assets?.totalFixedAssets)}</TableCell>
                                                </TableRow>
                                                <TableRow className="font-black bg-emerald-100">
                                                    <TableCell>TOTAL ASSETS</TableCell>
                                                    <TableCell className="text-right font-mono text-emerald-700">{formatCurrency(balanceSheetData.assets?.totalAssets)}</TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
                                    </CardContent>
                                            ...
                                            <Table>
                                                <TableBody>
                                                    <TableRow className="bg-red-50/50 font-bold">
                                                        ...
                                                        <TableCell className="text-right font-mono">{formatCurrency(balanceSheetData.liabilities?.totalCurrentLiabilities)}</TableCell>
                                                    </TableRow>
                                                    {balanceSheetData.liabilities?.currentLiabilities?.slice(0, 3).map((liab: any, idx: number) => (
                                                        <TableRow key={idx}>
                                                            ...
                                                            <TableCell className="text-right font-mono text-sm">{formatCurrency(liab.amount)}</TableCell>
                                                        </TableRow>
                                                    ))}
                                                    <TableRow className="font-black bg-red-100">
                                                        <TableCell>TOTAL LIABILITIES</TableCell>
                                                        <TableCell className="text-right font-mono text-red-700">{formatCurrency(balanceSheetData.liabilities?.totalLiabilities)}</TableCell>
                                                    </TableRow>
                                                </TableBody>
                                            </Table>
                                        </CardContent>
                                                ...
                                                <TableBody>
                                                    {balanceSheetData.equity?.capital?.slice(0, 3).map((cap: any, idx: number) => (
                                                        <TableRow key={idx}>
                                                            ...
                                                            <TableCell className="text-right font-mono text-sm">{formatCurrency(cap.amount)}</TableCell>
                                                        </TableRow>
                                                    ))}
                                                    <TableRow>
                                                        <TableCell className="text-sm text-zinc-500">Retained Earnings</TableCell>
                                                        <TableCell className="text-right font-mono text-sm">{formatCurrency(balanceSheetData.equity?.retainedEarnings)}</TableCell>
                                                    </TableRow>
                                                    <TableRow className="font-black bg-blue-100">
                                                        <TableCell>TOTAL EQUITY</TableCell>
                                                        <TableCell className="text-right font-mono text-blue-700">{formatCurrency(balanceSheetData.equity?.totalEquity)}</TableCell>
                                                    </TableRow>
                                                </TableBody>
                                            </Table>
                                        </CardContent>
                                    ...
                                    <Card className="border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                                        ...
                                        <CardContent>
                                            <div className={`text-2xl font-black ${cashFlowData.netIncreaseInCash >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                                                {formatCurrency(cashFlowData.netIncreaseInCash)}
                                            </div>
                                        </CardContent>
                                    </Card>
                                </div>
                                        ...
                                        <Table>
                                            <TableBody>
                                                ...
                                                <TableRow>
                                                    ...
                                                    <TableCell className="text-right font-mono">{formatCurrency(cashFlowData.operatingActivities?.netIncome)}</TableCell>
                                                </TableRow>
                                                ...
                                                <TableRow>
                                                    ...
                                                    <TableCell className="text-right font-mono text-emerald-700">{formatCurrency(cashFlowData.endingCash)}</TableCell>
                                                </TableRow>
                                            </TableBody>
                                        </Table>
                                    </CardContent>
