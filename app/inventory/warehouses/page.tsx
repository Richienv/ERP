...
const WAREHOUSES = [
  { id: "WH-A", name: "Gudang A - Bahan Baku", type: "Raw Material", location: "Building 1, Floor 1", manager: "Budi Santoso", staff: 12, capacity: 85, items: 12500, phone: "+62 812 3456 7890" },
  { id: "WH-B", name: "Gudang B - Barang Jadi", type: "Finished Goods", location: "Building 2, Floor 1", manager: "Siti Aminah", staff: 18, capacity: 62, items: 8500, phone: "+62 812 9876 5432" },
  { id: "WH-C", name: "Gudang C - Distribusi", type: "Distribution", location: "Building 2, Loading Dock", manager: "Rudi Hartono", staff: 8, capacity: 40, items: 2300, phone: "+62 812 1122 3344" },
];
        ...
        {WAREHOUSES.map((wh) => (
          <Card key={wh.id} className="border border-black shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col">
              ...
              <div className="flex justify-between items-start">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <Badge variant="outline" className="border-black font-mono text-xs bg-white text-black">{wh.id}</Badge>
                    <Badge className="bg-black text-white hover:bg-zinc-800">{wh.type}</Badge>
                  </div>
                  <CardTitle className="text-xl font-black uppercase leading-tight">{wh.name}</CardTitle>
                ...
                <div className="p-3 border border-black/10 rounded-lg bg-zinc-50/50">
                  ...
                  <div className="flex items-center gap-2">
                    <Avatar className="h-6 w-6 border border-black/20">
                      <AvatarFallback className="text-[10px] bg-white">{wh.manager.substring(0, 2)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="text-sm font-bold leading-none">{wh.manager}</p>
            ...
            <CardFooter className="pt-4 border-t border-black/5 bg-zinc-50">
              <Button variant="outline" className="w-full border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[1px] hover:translate-y-[1px] uppercase font-bold text-xs h-9">
                View Layout Map
              </Button>
            </CardFooter>
          </Card>
