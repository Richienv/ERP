...
export function WarehousesClient({ warehouses }: WarehousesClientProps) {
    const stats = {
        ...
        totalItems: warehouses.reduce((acc: number, w: any) => acc + (w.items || 0), 0),
        avgUtilization: warehouses.length > 0
            ? Math.round(warehouses.reduce((acc: number, w: any) => acc + (w.utilization || 0), 0) / warehouses.length)
            : 0,
        totalStaff: warehouses.reduce((acc: number, w: any) => acc + (w.staff || 0), 0),
    }
            ...
            <div className="border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] overflow-hidden bg-white dark:bg-zinc-900 rounded-none">
                <div className="px-6 py-4 flex items-center justify-between border-l-[6px] border-l-amber-400">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-xl font-black uppercase tracking-tight text-zinc-900 dark:text-white">Manajemen Gudang</h1>
                            <p className="text-zinc-400 text-xs font-medium mt-0.5">Monitor lokasi penyimpanan, utilisasi, dan staff gudang</p>
                        </div>
                    </div>
            ...
            <div className="bg-white dark:bg-zinc-900 border-2 border-black shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] overflow-hidden rounded-none">
                <div className="grid grid-cols-2 md:grid-cols-4">
                    <div className="relative p-4 md:p-5 md:border-r-2 border-b-2 md:border-b-0 border-zinc-100 dark:border-zinc-800">
                        ...
                        <div className="flex items-center gap-2 mb-2"><Warehouse className="h-4 w-4 text-zinc-400" /><span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Total Gudang</span></div>
                        <div className="text-2xl md:text-3xl font-black tracking-tighter text-amber-600">{stats.total}</div>
                        <div className="flex items-center gap-1 mt-1.5"><span className="text-[10px] font-bold text-amber-600">{stats.active} Aktif</span></div>
                    </div>
                    <div className="relative p-4 md:p-5 md:border-r-2 border-b-2 md:border-b-0 border-zinc-100 dark:border-zinc-800">
                        <div className="absolute top-0 left-0 right-0 h-1 bg-violet-400" />
                        <div className="flex items-center gap-2 mb-2"><Activity className="h-4 w-4 text-zinc-400" /><span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Avg. Utilisasi</span></div>
                        <div className="text-2xl md:text-3xl font-black tracking-tighter text-violet-600">{stats.avgUtilization}%</div>
                        <div className="flex items-center gap-1 mt-1.5"><Progress value={stats.avgUtilization} className="h-1.5 w-16 bg-violet-100 rounded-none" indicatorClassName="bg-violet-500" /></div>
                    </div>
                    <div className="relative p-4 md:p-5 md:border-r-2 border-b-2 md:border-b-0 border-zinc-100 dark:border-zinc-800">
                        <div className="absolute top-0 left-0 right-0 h-1 bg-blue-400" />
                        <div className="flex items-center gap-2 mb-2"><Boxes className="h-4 w-4 text-zinc-400" /><span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Item Disimpan</span></div>
                        <div className="text-2xl md:text-3xl font-black tracking-tighter text-blue-600">{(stats.totalItems / 1000).toFixed(1)}k</div>
                        <div className="flex items-center gap-1 mt-1.5"><span className="text-[10px] font-bold text-blue-600">Unit total</span></div>
                    </div>
                    <div className="relative p-4 md:p-5">
                        <div className="absolute top-0 left-0 right-0 h-1 bg-zinc-400" />
                        <div className="flex items-center gap-2 mb-2"><UserCheck className="h-4 w-4 text-zinc-400" /><span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Total Staff</span></div>
                        <div className="text-2xl md:text-3xl font-black tracking-tighter text-zinc-900 dark:text-white">{stats.totalStaff}</div>
                        <div className="flex items-center gap-1 mt-1.5"><span className="text-[10px] font-bold text-zinc-400">Pegawai aktif</span></div>
                    </div>
                </div>
            </div>
                        ...
                        <Card key={wh.id} className="border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] transition-all flex flex-col rounded-none overflow-hidden group bg-white dark:bg-zinc-900">
                            ...
                            <CardContent className="pt-5 flex-1 space-y-5 px-4 pb-5">
                                <div className="space-y-1.5">
                                    ...
                                    <Progress value={wh.utilization} className="h-2.5 border-2 border-black/10 bg-zinc-100 rounded-none" indicatorClassName={wh.utilization > 85 ? "bg-red-500" : "bg-black"} />
                                    <p className="text-[10px] font-mono text-zinc-400 text-right font-bold mt-0.5">{wh.items.toLocaleString()} / {wh.capacity.toLocaleString()} Units</p>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="p-2.5 border border-zinc-200 rounded-none bg-zinc-50/50">
                                        <div className="flex items-center gap-1.5 mb-2 text-zinc-400"><Users className="h-3.5 w-3.5" /><span className="text-[9px] font-black uppercase tracking-wider">Manager</span></div>
                                        <div className="flex items-center gap-2">
                            ...
                            <CardFooter className="pt-3 pb-4 px-4 border-t-2 border-black bg-zinc-50/50 flex gap-2">
                                <Button asChild variant="default" className="flex-1 bg-white text-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:bg-zinc-50 hover:shadow-none hover:translate-y-[1px] uppercase font-black text-[10px] tracking-wider h-8 rounded-none">
                                    <Link href={`/inventory/warehouses/${wh.id}`}><LayoutGrid className="mr-1.5 h-3.5 w-3.5" />Detail Gudang</Link>
                                </Button>
                                <div className="shrink-0"><WarehouseStaffDialog warehouseId={wh.id} warehouseName={wh.name} triggerMode="manager" /></div>
                            </CardFooter>
                        </Card>
                    ))}
                </div>
