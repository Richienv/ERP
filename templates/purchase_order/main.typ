...
#set text(font: ("Helvetica", "Arial", "sans-serif"), size: 10pt, fill: brand-colors.text, lang: "en")

// ============================================
// HELPER FUNCTIONS
// ============================================
#let format-address(addr) = {
  if addr == none or addr == "" { return "No address provided" }
...
// ============================================
#let po-number = get-field(data, "po_number", default: "NO-PO-NUMBER")
#let po-date = get-field(data, "date", default: "2025-01-30")
#let currency = get-field(get-field(data, "summary", default: (:)), "currency", default: "USD")

#let vendor = get-field(data, "vendor", default: (:))
#let vendor-name = get-field(vendor, "name", default: "No Vendor Name")
#let vendor-address = get-field(vendor, "address", default: "No Address")
#let vendor-tax-id = get-field(vendor, "tax_id", default: "N/A")
#let vendor-contact = get-field(vendor, "contact", default: "N/A")
#let vendor-email = get-field(vendor, "email", default: "no-email@example.com")

#let ship-to = get-field(data, "ship_to", default: (:))
#let ship-warehouse = get-field(ship-to, "warehouse", default: "No Warehouse")
#let ship-address = get-field(ship-to, "address", default: "No Address")
...
#grid(
  columns: (1fr, 1fr),
  gutter: 24pt,
  box(fill: brand-colors.background, radius: 4pt, inset: 12pt, width: 100%)[
...
#table(
  ...
  fill: (x, y) => if y == 0 { brand-colors.primary } else { if calc.even(y) { brand-colors.background } else { white } },
  ...
  text(size: 8pt, weight: "bold", fill: white, upper("SKU")),
  text(size: 8pt, weight: "bold", fill: white, upper("Description")),
  text(size: 8pt, weight: "bold", fill: white, upper("Qty")),
  text(size: 8pt, weight: "bold", fill: white, upper("Unit Price")),
  text(size: 8pt, weight: "bold", fill: white, upper("Total")),
